...
netflix<-read.csv("RESULTS_20757532_2021-08-06.csv")
drop<-c("Prev.Type")
...
```
From the main effect plots above, it seems that browsing time changes significantly when we change the factor level from "low" to "high" for each of the factors. Hence, all of our factors seems to influence Browsing time significantly. Let us perform formal hypothesis tests to confirm this statement.

We are going to fit a linear regression model with linear predictor $\beta_0 + \beta_1x_1 + \beta_2x_2 + \beta_3x_3$. To test the significance of a factor, we are going to test three hypothesis which are $H_0: \beta_j=0$ for j=1,2,3. These hypothesis determine whether the main effects are significantly different from zero hence conclude whether the factors influence the response. Since we tested multiple hypothesis, in particular 3, we need to address the multiple testing problem. However, even with the strictest correction which is Bonferroni correction, our adjusted observed p-values(6.722183e-28, 2.394960e-62, 8.359811e-48) for our hypothesis, are practically zero.

#### Conclusion:
Since we performed a fractional factorial experiment, we must be aware of the confounding effects in our experiment, in particular, all of our main effects are confounded with interaction effects as detailed in the design process. However, based on the Principle of effect sparsity, we know that these interaction terms are less likely to be significant than our main effect terms. Our adjusted p-values, found in analysis portion of the experiment above, are extremely close to zero hence they are all significant. So, based on the Principle of effect sparsity, we conclude that all main effects significantly influences our response. Hence, Tile Size, Match Score and Preview Length are all significant factors.
...
```
Note, since Match Score is an integer, the value of a is rounded up to be 1.5. Based on the conditions defined and data collected, we fitted a full second order model. We, then, proceeded to calculate the stationary point of our estimated full order model and found out that the natural optimum conditions are 
(0.332,66, 90) for tile size, match score and preview length respectively.

#### Conclusion:
The estimated average browsing time from our model is 12.3 seconds with confidence interval [12.09459 12.50825]. This is a massive improvement of approximately 20 percent from our default set of values of the factors which gave an average browsing time of 15 seconds. One limitations is that we assumed that this is the global optimum which is a practical assumption as otherwise we need to collect additional data and perform Phase II again to see if we can find another valley that may contain another optimum small localized region. However, a 20 percent improvement in average browsing time will generate Netflix more revenue than it was generating before which is a positive step for the company.
